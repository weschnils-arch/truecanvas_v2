      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div ref={headerRef} className="text-center mb-16 md:mb-20 opacity-0">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-normal text-charcoal tracking-tight">
            {whyChooseMeConfig.titleRegular} <span className="font-serif italic font-normal text-gold">{whyChooseMeConfig.titleItalic}</span>
          </h2>
        </div>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-3 gap-6 md:gap-8">
          {whyChooseMeConfig.featureCards.map((card, index) => (
            <div key={index} className="feature-card-image opacity-0 group">
              <div className="relative aspect-[3/4] rounded-lg overflow-hidden bg-paper ink-wash-frame">
                <img
                  src={card.image}
                  alt={card.imageAlt}
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <p className="text-white/90 font-normal text-lg mb-2">
                    {card.title}
                  </p>
          ...
          {whyChooseMeConfig.stats.length > 0 && (
            <div
              ref={statsRef}
              className="feature-card-stats opacity-0 bg-paper rounded-lg p-8 md:p-10 flex flex-col justify-between border border-charcoal/5"
            >
              <div>
                {whyChooseMeConfig.statsLabel && (
                  <p className="font-mono text-[11px] tracking-[0.18em] text-gold mb-8">
                    {whyChooseMeConfig.statsLabel}
                  </p>
                )}
                <div className="space-y-8">
                  {whyChooseMeConfig.stats.map((stat, index) => (
                    <div key={index} className="border-b border-charcoal/10 pb-6 last:border-0">
                      <p className="text-4xl md:text-5xl font-normal text-charcoal tracking-tight">
                        <Counter
                          end={stat.value}
                          suffix={stat.suffix}
        ...
        {whyChooseMeConfig.wideImage && (
          <div ref={wideRef} className="mt-16 md:mt-24 relative rounded-lg overflow-hidden group opacity-0 ink-wash-frame">
            ...
            {(whyChooseMeConfig.wideTitle || whyChooseMeConfig.wideDescription) && (
              <div className="wide-text-overlay absolute bottom-8 left-8 md:bottom-12 md:left-12 max-w-md opacity-0">
                {whyChooseMeConfig.wideTitle && (
                  <p className="text-white/90 font-normal text-2xl md:text-3xl mb-3">
                    {whyChooseMeConfig.wideTitle}
                  </p>
                )}
